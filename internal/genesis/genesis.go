...
1. 世界观（2-3 句）
   ...
   绝对禁止：世界观中不得包含任何关于阵营归属、角色身份、谁是好人谁是坏人的信息。
   世界观只描述场景和背景故事，不涉及游戏机制。用户提供的场景要求中的阵营指定等meta信息不得出现在世界观描述中。
...
5. 严格区分"创世指令"和"游戏内容"：用户关于阵营分配、角色指定等要求是给你（上帝）的meta指令，你在执行时必须遵守，但这些信息绝对不能出现在setting、persona等游戏内容中。setting只写叙事背景，persona只写人物性格，任何人读了都不应该能推断出谁是狼人谁是好人。
